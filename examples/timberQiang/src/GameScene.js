...
var MainGame = flax.MovieClip.extend({
    ...
    onEnter:function(){
        ...
        this.treeBatch = cc.SpriteBatchNode.create(cc.path.changeBasename(res.assets, ".png"), 8);
    ...
    initTree:function(){
        var t = flax.assetsManager.createDisplay(res.assets, "tree0",{parent: this.treeBatch}, true);
        t.x = TREE_X;
        t.side = 0;
    ...
    onFailed:function(){
        var death = flax.assetsManager.createDisplay(res.assets,"death", {parent: this, zIndex:200}, true);
        death.x = (this.player.side == 1) ? 150 : 500;
        death.y = 240;
        death.runAction(cc.MoveTo.create(0.3, cc.p(death.x, 150)));
        this.player.destroy();
        this.scheduleOnce(function(){
            flax.assetsManager.createDisplay(res.assets,"ResultPanel",{parent: this, zIndex:1000});
        },0.5);
        flax.playSound(music.fail);
        flax.inputManager.removeListener(null, this.onTouch);
    },
    showTreeAnim:function(){
        var anim = flax.assetsManager.createDisplay(res.assets,"treeAnim",{parent: this, zIndex:2,autoDestroyWhenOver:true}, true);
        anim.fps = 60;
    ...
    createTreeSegment:function(tn){
        ...
        var t = flax.assetsManager.createDisplay(res.assets, tn,{parent: this.treeBatch}, true);
        t.x = TREE_X;
        t.y = this._currentY;
        t.side = i;
    }
...
var GameScene = cc.Scene.extend({
    onEnter:function(){
        ...
        var ui = flax.assetsManager.createDisplay(res.assets, "MainGame");
        this.addChild(ui);
    },
    onExit:function(){
        this._super();
...
var MenuScene = cc.Scene.extend({
    onEnter:function(){
        this._super();
        var ui = flax.assetsManager.createDisplay(res.assets, "MainMenu");
        this.addChild(ui);
        flax.inputManager.addListener(ui.startBtn, function(){
           flax.replaceScene("game");
        });
        ui.player.play();
    }
})
